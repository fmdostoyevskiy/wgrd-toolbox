...
- **Styling:** mostly inline style objects built from `BROWSER_TOKENS`/`BMono`/`V2_THEMES` (`units-core/constants/theme.js`), plus per-tool `layout.css`/`styles.css`. The global CSS variables in `src/index.css` mirror the JS tokens. UI scale is a CSS `zoom` driven by the `--wrd-zoom` variable. `zoomStore.js` sets it from localStorage, and each tool's `main.jsx` imports it for that side effect.
