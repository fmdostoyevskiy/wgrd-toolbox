...
import { CombatApp } from './CombatApp.jsx';
  ...
  return (
    <div style={{
      ...
      <div style={{ color: t.accent, fontSize: 13, letterSpacing: '0.24em', fontWeight: 600 }}>
        COMBAT<span style={{ color: t.accent, marginLeft: 4 }}>/WRD</span>
      </div>
      <div>{message}</div>
    </div>
  );
...
async function init() {
  ...
  root.render(
    <CombatApp roster={roster} units={units}
      defaultA={findByName(roster, 'LEOPARD 2A4', defaultId)}
      defaultB={findByName(roster, 'BMP-3', defaultId)} />
  );
}
