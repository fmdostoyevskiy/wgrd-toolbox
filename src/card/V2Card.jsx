  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 18px 18px' }}>
        ...
        <GeneralSection unit={unit} s={s} />
        {!isFob && hasMobility(unit) && <MobilitySection unit={unit} s={s} />}
        {!isFob && hasOptics(unit)   && <OpticsSection   unit={unit} s={s} />}
        {showArmor   && <ArmorSection    armor={unit.armor} s={s} />}
        {showWeapons && <ArmamentSection weapons={unit.weapons} vetMul={vet.accMul} s={s} />}
      </div>
    </div>
  );
}
