  ...
  return (
    <div style={{ margin: '10px 0 12px', border: `1px solid ${s.rule}`, background: s.paper }}>
      ...
      <div className="sr" style={{ padding: '6px 12px 8px' }}>
        ...
        {w.category !== 'Bomb' && (
          <>
            {w.aimTime != null && <DotRow label="Aim Time" value={`${w.aimTime} s`} s={s} dense />}
            <DotRow
        ...
        {rearm && <DotRow label="Rearm Time" value={rearm.value} tooltip={rearm.tooltip} s={s} dense />}
        {supply && <DotRow label="Supply"     value={supply.value} tooltip={supply.tooltip} s={s} dense />}
      </div>
    </div>
  );
}
