...
| `baseAccuracyOnly` | show only the base accuracy and stabilizer, without the veterancy-adjusted value |
...
- `hit.js`: `hitChance(w, target, { distance, vetIdx, morale, mode, shooter })` → `{ ok, reason, hit, frac, range, rangeLabel, steps }`. `steps` is the per-modifier breakdown. Shots at planes, and from a `shooter` that's a plane, get no range bonus.
...
- `volley.js`: several weapons at once. `turretGroups(weapons)` → `[{ turret, idx }]` (weapons on one turret can't fire together), `effectiveSelection(chosen, groups, usable)` (one usable weapon per chosen group), `shownWeapon(group, sel, usable)` (the weapon a group's box shows), `volley([{ times, p, dmg }], hp)` → `{ steps, at(t), killAt(p) }`: the combined damage distribution after every shot, each step `{ t, mean, worst, best, kill, parts }` (`parts` splits `mean` between the weapons).

## Compare UI

`compare/` holds the pieces the accuracy and combat tools share: `UnitPicker` (the selected unit, which turns into a search box with the unit list dropdown when clicked; `beside` puts e.g. the weapon tabs on the same row), `SeriesChart` (0–100% per series along an x axis, with optional dotted low/high lines, that doubles as the control for x; `aside` puts a node such as a `VerticalSlider` beside the plot) and its distance preset `DistanceChart` (`useAxisControl` and `AxisScale` are its click/drag/keyboard x control and tick scale, for other charts), `CompareHeader`, `CrewControls` (veterancy and morale gauges, built on `LevelMeter`), `SquarePicker` (a row of small squares, name over a colored value) and `ArmorPicker` (armor squares with pickable facings), `ControlRow`, `WeaponTabs` (`max` caps the tabs, the last one opening a menu of the rest), `WeaponGroups` (one box per turret group, three to a row, each toggling its weapon, with a menu for turrets that have several and an optional details checkbox, one turret at a time), `CalcSteps` and helpers (`themeOf`, `distanceScale`, `effectiveWeapon`, …). Tools that use them import `@units-core/compare/compare.css` in their `main.jsx`.
