...
export const DATASETS = {
  ...
  tanks: {
    ...
    columns: [
      ...
      { key: 'ammoType',   label: 'AMMO',     type: 'text',      width: 65,  heat: null   },
      { key: 'he',         label: 'HE',       type: 'num',       width: 65,  heat: 'high' },
      { key: 'shot',       label: 'SHOT RLD', type: 'num',       width: 80,  heat: 'low'  },
      { key: 'auto',       label: 'AL',       type: 'bool-good', width: 55,  heat: null   },
      { key: 'acc',        label: 'ACC',      type: 'pct',       width: 65,  heat: 'high' },
      { key: 'stab',       label: 'STAB',     type: 'pct',       width: 65,  heat: 'high' },
    ...
    transform(u) {
      ...
      return {
        ...
        ap:         wf(gun, 'ap'),
        he:         wf(gun, 'dmg'),
        shot:       wf(gun, 'shotReload') ?? wf(gun, 'salvoReload'),
  ...
  helomissileaa: {
    ...
    transform(u) {
      ...
      return {
        ...
        acc:      wf(m, 'acc'),
        stab:     wf(m, 'stab'),
        mspd:     wf(m, 'missileSpeedHE') ?? wf(m, 'missileSpeed'),
        salvoLen: wf(m, 'salvoLen'),
        ammo:     wf(m, 'ammo'),
        fnf:      m?.tag?.includes('FnF') ?? false,
        radar:    m?.tag?.includes('RAD') ?? false,
  ...
  rocketpodhelos: {
    ...
    presets: [
      { label: 'TOUGH',      filters: { health:   { min: '6'   } } },
      { label: 'HIGH DMG',   filters: { dmg:      { min: '2'   } } },
      { label: 'LONG SALVO', filters: { salvoLen: { min: '10'  } } },
      { label: 'MED OPTICS', filters: { optics:   { min: '80'  } } },
      { label: 'RECON',      filters: { optics:   { min: '120' } } },
    ],
    columns: [
      ...N,
  ...
  asfs: {
    ...
    columns: [
      ...
      { key: 'm1Shot',   label: 'M1 SHOT',    type: 'num',       width: 70,  heat: 'low'  },
  ...
  atgmvehicles: {
    ...
    columns: [
      ...
      { key: 'stab',        label: 'STAB',      type: 'pct',  width: 65,  heat: 'high' },
      { key: 'rng',         label: 'GND RNG',   type: 'num',  width: 85,  heat: 'high' },
      { key: 'missileAccel',label: 'MS ACCEL',  type: 'num',  width: 85,  heat: 'high' },
      { key: 'mspd',        label: 'MS SPD',    type: 'num',  width: 80,  heat: 'high' },
      { key: 'salvoLen',    label: 'SALVO LEN', type: 'num',  width: 90,  heat: 'high' },
      { key: 'ammo',        label: 'AMMO',      type: 'num',  width: 65,  heat: 'high' },
      { key: 'speed',       label: 'SPEED',     type: 'num',  width: 70,  heat: 'high' },
    ...
    transform(u) {
      ...
      return {
        ...
        stab:         wf(m, 'stab'),
        rng:          wf(m, 'rng_g'),
        missileAccel: wf(m, 'missileAccelAP') ?? wf(m, 'missileAccel'),
        mspd:         wf(m, 'missileSpeedAP') ?? wf(m, 'missileSpeed'),
        salvoLen,
        ammo:         wf(m, 'ammo'),
        speed:        u.speed ?? null,
        stealth:      u.stealth ?? null,
